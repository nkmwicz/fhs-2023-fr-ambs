export function colorToRGBA(color) {
  ...
  if (color[0] === "#") {
    ...
    return [
      ...
      color.length > 7 ? parseInt(color.substr(7, 2), 16) / 255 : 255,
    ];
  }
  ...
  if (color.indexOf("rgb") === 0) {
    if (color.indexOf("rgba") === -1) color += ",255  "; // convert 'rgb(R,G,B)' to 'rgb(R,G,B)A' which looks awful but will pass the regxep below
